...
function AddFoodView({ open, onClose, date, onFoodSelect }: AddFoodViewProps) {
  ...
  const renderFoodItem = (food: Food) => (
    <Card key={food.id} className="add-food-item" onClick={() => selectFood(food)} role="button" tabIndex={0}>
      <div className="add-food-item-row">
        {food.image_url ? (
      ...
      <span className="add-food-item-kcal" data-emphasis="weak">
        {food.kcal_per_100g} kcal /100{food.default_unit}
      </span>
    </Card>
  )
        ...
        {isSearching ? (
          ...
          <>
            <div className="add-food-tabs">
              <Button
                data-material={activeTab === 'recent' ? 'inverted' : undefined}
                data-container-contrast={activeTab === 'recent' ? 'max' : undefined}
                onClick={() => setActiveTab('recent')}
              >
                Letzte
              </Button>
              <Button
                data-material={activeTab === 'recipes' ? 'inverted' : undefined}
                data-container-contrast={activeTab === 'recipes' ? 'max' : undefined}
