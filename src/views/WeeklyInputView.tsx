...
function WeeklyInputView({ open, onClose }: WeeklyInputViewProps) {
  ...
  const [entries, setEntries] = useState<StepFlowEntry[]>(() =>
    ZONE_CONFIG.map(({ zone, label }) => ({ zone, label, value: null, skipped: false }))
  )
  ...
  const loadPreviousWeek = useCallback(async () => {
    try {
      const prev = await getWeeklyMeasurement(getPreviousWeekStart(weekStart))
      setPreviousWeek(prev)
    } catch { setPreviousWeek(undefined) }
  }, [weekStart])
  ...
  const loadExistingData = useCallback(async () => {
    try {
      ...
      if (existing) {
        setEntries(ZONE_CONFIG.map(({ zone, label }) => {
          const val = existing[zone]
          return { zone, label, value: val != null ? val : null, skipped: val == null }
        }))
      }
    } catch { /* keep defaults */ }
  }, [weekStart])
  ...
  }, [open, loadPreviousWeek, loadExistingData])
  ...
  const handleNext = () => {
    ...
    if (raw === '') { setError('Bitte einen Wert eingeben oder überspringen'); return }
    const normalized = normalizeDecimal(raw)
    const value = Number(normalized)
    if (isNaN(value) || !validateCircumference(value)) {
      setError(`Wert muss zwischen ${CIRCUMFERENCE_MIN} und ${CIRCUMFERENCE_MAX} cm liegen`); return
    }
    const rounded = roundToOneDecimal(value)
    hasNavigated.current = true
    setDirection('forward')
    setEntries(prev => { const next = [...prev]; next[step - 1] = { ...next[step - 1], value: rounded, skipped: false }; return next })
    setCurrentInput('')
    setError(undefined)
    setStep(step + 1)
  }
  ...
  const handleSkip = () => {
    hasNavigated.current = true
    setDirection('forward')
    setEntries(prev => { const next = [...prev]; next[step - 1] = { ...next[step - 1], value: null, skipped: true }; return next })
    setCurrentInput('')
    setError(undefined)
    setStep(step + 1)
  }
  ...
  const handleConfirm = async () => {
    try {
      const measurement: WeeklyMeasurement = { date: weekStart, updatedAt: new Date().toISOString() }
      for (const entry of entries) {
        if (!entry.skipped && entry.value !== null) measurement[entry.zone] = entry.value
      }
      ...
      try {
        ...
        const [goals, streaks, earned] = await Promise.all([getAllGoals(), getStreaks(), getEarnedMilestones()])
        await evaluateMilestones({ goals, streaks, dailyMeasurements: allData.dailyMeasurements, weeklyMeasurements: allData.weeklyMeasurements, earnedMilestones: earned })
        detectNonScaleVictories(allData.dailyMeasurements, allData.weeklyMeasurements)
      } catch { /* gamification non-blocking */ }
    } catch { /* save error */ }
  }

  const dialogTitle = step <= TOTAL_STEPS ? `Messung (${step}/${TOTAL_STEPS})` : 'Zusammenfassung'

  return (
    <Dialog title={dialogTitle} onClose={onClose} open={open}>
        ...
        {step <= TOTAL_STEPS ? (
          ...
          >
            <StepFlowScreen
              ...
              value={currentInput}
              onChange={(val) => { setCurrentInput(val); setError(undefined) }}
              onNext={handleNext}
              onSkip={handleSkip}
              onCancel={onClose}
          ...
          >
            <StepFlowSummary
              ...
              onBack={() => { hasNavigated.current = true; setDirection('backward'); setStep(1) }}
            />
          </motion.div>
        )}
      </AnimatePresence>
